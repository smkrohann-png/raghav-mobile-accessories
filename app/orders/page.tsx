            ...
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              {orders.length > 1 ? (
                <div className="mt-6 flex flex-wrap gap-2">
                  {orders.map((order) => (
                    <button className={`rounded-full px-4 py-2 text-sm font-bold ${order.id === selectedId ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-700"}`} key={order.id} onClick={() => setSelectedId(order.id)}>
                      {order.id}
                    </button>
                  ))}
                </div>
              ...
              {selectedOrder.shiprocketAwbCode && (
                <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/20 p-5 space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <Truck className="h-5 w-5 text-emerald-600" />
                      <p className="font-bold text-slate-900">Shiprocket Shipment Tracking</p>
                    </div>
                    <span className="rounded bg-emerald-100 text-emerald-850 px-2 py-0.5 text-xs font-black uppercase tracking-[0.05em]">
                      Live Sync
                    </span>
                  </div>
